...
By contributing to Luna Slimes, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Luna Slimes.

If there is more than one acceptable spelling of the same word, first check if one word is already
being used in Luna Slimes, FrozenLib, or in Mojang's Mappings, and if not, use the spelling that is most commonly used.
...
It allows to separate the actual name of the variable and the namespace, `lunaSlimes`.

Fields marked as `@Unique` must be prefixed with `lunaSlimes$` or `LUNASLIMES$` if the field is static and final.

Methods marked with `@Unique` must be prefixed with `lunaSlimes$`.

Injector or modifier methods do not need to be prefixed. Fabric takes care of that.

## Licensing & DCO

Luna Slimes is licensed under [FrozenBlock Modding Oasis License][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE.md "Luna Slimes license file"
